...
pub mod tests {
    ...
    use crate::{
        kernel::matmul::test_utils::{
            assert_equals, assert_equals_range, create_empty, range_tensor, range_tensor_generic,
        },
        JitRuntime,
    };
    ...
    /// Exported test
    pub fn compute_loop_mimic_offseted_warp_test<R: JitRuntime>(device: &R::Device) {
    ...
    /// Exported test
    pub fn compute_loop_cmma_offseted_warp_test<R: JitRuntime>(device: &R::Device) {
        let lhs = range_tensor_generic::<f16, R>(32, 16, device);
        let rhs = range_tensor_generic::<f16, R>(16, 32, device);
        let results = create_empty::<R>(32, 32, device);
        let cube_dim = CubeDim::new(1, 32, 1);
        let cube_count = CubeCount::Static(1, 1, 1);
